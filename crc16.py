
CCITT_1 = 0
CCITT   = 1

class Crc16(object):
	'''
	Crc16 ccitt or ccitt_1
	'''
	def __init__(self):
		self.nCrcHi = 0xff
		self.nCrcLo = 0xff

		self.Crc16HiValue =\
			[]

		self.Crc16LoValue =\
			[]

	def crc16_init(self, flag):
		if flag == CCITT_1:
			self.nCrcHi, self.nCrcLo = 0xff, 0xff
		else:
			self.nCrcHi, self.nCrcLo = 0x0, 0x0

	def crc16(self, data, dlen):
		if dlen > 0:
			for i in range(dlen):
				try:
					n  = self.nCrcHi ^ data[i]
					self.nCrcHi = self.nCrcLo ^ self.Crc16HiValue[n]
					self.nCrcLo = self.Crc16LoValue[n]
				except Exception as e:
					self.nCrcHi = 0xff
					pass

		return self.nCrcHi << 8 | self.nCrcLo

	def reen_crc16(self, flag, data, dlen):
		Hi, Lo = 0x0, 0x0
		if dlen > 0:
			if flag==CCITT_1:
				Hi, Lo = 0xff, 0xff
			for i in range(dlen):
				try:
					n = Hi ^ data[i]
					Hi = Lo ^ self.Crc16HiValue[n]
					Lo = self.Crc16LoValue[n]
				except Exception as e:
					Hi = 0xff
					pass

		return (Hi << 8 | Lo)

	def check_crc16(self, flag, data):
		dlen = len(data)

		if len <= 2:
			return False
		else:
			return self.reen_crc16(flag, data, dlen) == 0


if __name__ == '__main__':
	data = [0xA3,0x0D,0xF3,0x20,0xEA,0x60,0x01,0x00,0x00,0x02,0x00,0x3d,0xdd]
	crc = Crc16()
	crc.crc16_init(CCITT_1)
	n1 = crc.crc16(data, len(data) - 2)
	n2 = crc.reen_crc16(CCITT_1, data, len(data) - 2)
	if crc.check_crc16(CCITT_1, data):
		print("data check success!")
	else:
		print("data check failed!")

	print('crc1 = {0:x},{1:x}'.format(n1, n2))
